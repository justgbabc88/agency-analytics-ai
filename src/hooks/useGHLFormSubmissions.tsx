  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch submissions with date filtering at database level for better performance
        let submissionsQuery = supabase
          .from('ghl_form_submissions')
          .select('*')
          .eq('project_id', projectId);

        // Apply date filtering at database level if dateRange is provided
        if (dateRange) {
        ...
        const { data: submissionsData, error: submissionsError } = await submissionsQuery
          ...
          .limit(5000); // Reasonable limit instead of fetching everything
