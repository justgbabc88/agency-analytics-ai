...
-- 4. Strengthen RLS policies for sensitive data with audit logging

-- Enhanced attribution data policy with logging
...
-- Enhanced tracking events policy
...
FOR INSERT WITH CHECK (
  -- Only allow completely anonymous tracking (no contact info)
...
FOR INSERT WITH CHECK (
  ...
  -- Log when contact info is being stored
  CASE WHEN (contact_email IS NOT NULL OR contact_phone IS NOT NULL OR contact_name IS NOT NULL)
    THEN (SELECT public.log_security_event(
      NULL, 'contact_data_stored', 'tracking_events', NEW.id, 
      jsonb_build_object('has_email', contact_email IS NOT NULL, 'has_phone', contact_phone IS NOT NULL),
      'warning'
    ) IS NOT NULL)
    ELSE true
  END
...
-- 5. Create data encryption functions for sensitive fields
...
AS $$
BEGIN
  -- Simple obfuscation for contact data (in production, use proper encryption)
  IF data IS NULL OR length(data) = 0 THEN
    RETURN NULL;
  END IF;
  
  -- Hash the data for basic protection
...
BEGIN
  window_start := date_trunc('hour', now()) + (extract(minute from now())::integer / p_window_minutes) * (p_window_minutes::text || ' minutes')::interval;
  ...
  WHERE identifier = p_identifier 
    ...
    AND window_start = date_trunc('hour', now()) + (extract(minute from now())::integer / p_window_minutes) * (p_window_minutes::text || ' minutes')::interval;
