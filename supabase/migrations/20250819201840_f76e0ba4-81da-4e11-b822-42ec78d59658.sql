...
-- =============================================================================
-- PHASE 2: MISSING RLS POLICIES - HIGH PRIORITY DATA PROTECTION
-- Add RLS policies to tables that currently have NONE (major security risk)
-- =============================================================================

-- Enable RLS on tables that are missing it
ALTER TABLE public.calendly_sync_health ENABLE ROW LEVEL SECURITY;
...
-- CALENDLY_SYNC_HEALTH: Restrict access to project owners only
CREATE POLICY "users_can_view_their_project_sync_health" ON public.calendly_sync_health
FOR SELECT 
TO authenticated
USING (
  project_id IN (
...
CREATE POLICY "service_can_manage_sync_health" ON public.calendly_sync_health
FOR ALL 
TO service_role
WITH CHECK (true);

-- PROJECT_DAILY_METRICS: Protect aggregated analytics data
CREATE POLICY "users_can_view_their_project_metrics" ON public.project_daily_metrics
FOR SELECT 
TO authenticated
USING (
  project_id IN (
...
CREATE POLICY "service_can_manage_daily_metrics" ON public.project_daily_metrics
FOR ALL 
TO service_role
WITH CHECK (true);
