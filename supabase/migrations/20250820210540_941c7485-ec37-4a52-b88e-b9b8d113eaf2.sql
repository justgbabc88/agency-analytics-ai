-- PHASE 1: Emergency PII Protection - Fix Critical RLS Policy Issues (Corrected)
...
CREATE POLICY "strict_calendly_events_insert" ON public.calendly_events
FOR INSERT
WITH CHECK (
  current_setting('role') = 'service_role' OR
  project_id IN (
...
CREATE POLICY "strict_calendly_events_update" ON public.calendly_events
FOR UPDATE
TO authenticated
USING (
  project_id IN (
...
CREATE POLICY "strict_calendly_events_delete" ON public.calendly_events
FOR DELETE
TO authenticated
USING (
  project_id IN (
...
CREATE POLICY "service_role_calendly_access" ON public.calendly_events
FOR ALL
TO service_role
USING (true)
WITH CHECK (true);