...
TO service_role
WITH CHECK (
  -- Service role can only insert for valid projects
  project_id IN (
    SELECT id FROM public.projects WHERE id IS NOT NULL
  )
);
...
TO anon
WITH CHECK (
  -- Only allow basic page views with no sensitive data
...
TO anon
WITH CHECK (
  -- Only allow session creation for valid projects
...
TO anon
USING (
  -- Only allow updates to sessions with valid projects
  project_id IN (
    SELECT id FROM public.projects WHERE id IS NOT NULL
  )
)
WITH CHECK (
  ...
  project_id = (SELECT project_id FROM public.tracking_sessions WHERE id = tracking_sessions.id)
);
