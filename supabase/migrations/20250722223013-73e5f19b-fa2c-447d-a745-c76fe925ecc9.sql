-- Run aggregation for missing recent days
DO $$
DECLARE
...
BEGIN
  ...
  FOR project_record IN SELECT id FROM projects LOOP
    -- Aggregate for each of the missing recent days
    FOR date_record IN SELECT generate_series(CURRENT_DATE - INTERVAL '4 days', CURRENT_DATE, '1 day'::interval)::date LOOP
      -- Only run for dates that don't have data yet or need refresh
